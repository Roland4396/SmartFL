...
def train_supernet(args):
    ...
    device = torch.device(f"cuda:{args.gpu_idx}" if torch.cuda.is_available() and args.use_gpu else "cpu")
    print(f"--> Using device: {device}")

    print(f"--> Loading dataset: {args.data}")
    train_set, val_set, test_set = get_datasets(args)
    train_loader, _, _ = get_dataloaders(args, args.batch_size, (train_set, val_set, test_set))
